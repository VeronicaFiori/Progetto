...
NOTE_DURATION = 0.5
TEMPERATURE = 1.0  # Più alto = più casuale (es. 1.2), più basso = più conservativo (es. 0.7)

def sample_with_temperature(preds, temperature=1.0):
    """Campiona un indice dalla distribuzione con temperatura."""
...
def generate_music(seed_sequence, model, length=100, temperature=1.0):
    """Genera nuova sequenza musicale usando temperature sampling."""
    generated = list(seed_sequence)
    for _ in range(length):
        input_seq = np.array(generated[-SEQUENCE_LENGTH:]).reshape(1, SEQUENCE_LENGTH)
        prediction = model.predict(input_seq, verbose=0)
        next_note = sample_with_temperature(prediction, temperature)
...
def sequence_to_midi(sequence, output_file, duration=0.5):
    """Converte una sequenza di pitch in file MIDI."""
    midi = pretty_midi.PrettyMIDI()
    instrument = pretty_midi.Instrument(program=0)
    start = 0
    for pitch in sequence:
        pitch = int(np.clip(pitch, 0, 127))  # Assicura pitch validi
...
def main():
    # Optional: imposta un seed per generazioni ripetibili
    np.random.seed(42)

    model = tf.keras.models.load_model(MODEL_PATH)
    seed_sequence = np.random.randint(0, VOCAB_SIZE, SEQUENCE_LENGTH)
    generated_sequence = generate_music(seed_sequence, model, length=200, temperature=TEMPERATURE)

    output_file = get_next_filename(OUTPUT_DIR)
    sequence_to_midi(generated_sequence, output_file, duration=NOTE_DURATION)
